# fix das querys de insert após correçaõ dos nomes das colunas
# de Costumer para Customer

insert into registerProduct (idCustomer, idProduct, idSupermarket, price, dateRegister) values
    (2, 7, 5, 2.90, now()), 	
    (3, 2, 1, 12.99, now()),
    (3, 14, 1, 17.50, now()),
    (3, 2, 1, 11.30, now()),
...
# deixando indexes UPDATE CASCADE e DELETE RESTRICT 
ALTER TABLE `sql10527490`.`registerProduct` 
DROP FOREIGN KEY `indx_idCustomer`,
DROP FOREIGN KEY `indx_idProduct`;
ALTER TABLE `sql10527490`.`registerProduct` 
ADD CONSTRAINT `indx_idCustomer`
  FOREIGN KEY (`idCustomer`)
  REFERENCES `sql10527490`.`customer` (`idCustomer`)
  ON DELETE RESTRICT
  ON UPDATE CASCADE,
ADD CONSTRAINT `indx_idProduct`
  FOREIGN KEY (`idProduct`)
  REFERENCES `sql10527490`.`product` (`idProduct`)
  ON DELETE RESTRICT
  ON UPDATE CASCADE;
  
  
# DAQUI PRA BAIXO JÁ ESTAVAM CORRETAS #
